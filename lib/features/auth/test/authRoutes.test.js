...
test('POST /logout tanpa token CSRF valid ditolak dan sesi tidak dihancurkan', async () => {
  await withApp(
    { session: { user: { id: 7, username: 'budi' }, _csrf: 'token-benar' } },
    async ({ base, session }) => {
      const res = await postForm(base, '/logout', { _csrf: 'token-salah' });
      // Kode menulis res.status(403).redirect('/dashboard'), tetapi res.redirect()
      // Express menimpa statusCode menjadi 302 — jadi 403 itu mati. Yang penting
      // secara keamanan tetap terpenuhi: sesi TIDAK dihancurkan. Dikunci apa adanya;
      // perbaikan status code masuk backlog D3.
      assert.equal(res.status, 302);
      assert.equal(res.headers.get('location'), '/dashboard');
