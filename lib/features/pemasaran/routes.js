...
import { bulanIndonesia, savePlansToDb, schedulePlanToRepliz, syncPlanReplizStatus } from './domain.js';
import { requireLogin } from '../auth/requireLogin.js';
import { sleep, randomBulkDelayMs } from './jobs.js';
import { getReplizAccounts } from '../../shared/repliz.js';
import { getChannel } from '../channels/index.js';
import { requestLogger } from '../../shared/logger.js';

export function registerPemasaranRoutes(
  app,
  {
    dbPool = pool,
    savePlans = savePlansToDb,
  ...
  app.get('/api/pemasaran', requireLogin, async (req, res) => {
    ...
    `);
      ...
      requestLogger(req, 'pemasaran').error({ err }, 'GET /api/pemasaran error');
  ...
  app.get('/api/pemasaran/:id', requireLogin, async (req, res) => {
    try {
      const result = await dbPool.query('SELECT * FROM pemasaran WHERE id = $1', [req.params.id]);
      if (result.rows.length === 0)
        return res.status(404).json({ error: 'Rencana pemasaran tidak ditemukan' });
      res.json(result.rows[0]);
    } catch (err) {
      requestLogger(req, 'pemasaran').error({ err }, 'GET /api/pemasaran/:id error');
  ...
  app.post('/api/pemasaran', requireLogin, async (req, res) => {
    try {
      const saved = await savePlans(req.body, dbPool);
      ...
      requestLogger(req, 'pemasaran').error({ err }, 'POST /api/pemasaran error');
  ...
  app.post('/api/pemasaran/repliz/schedule', requireLogin, async (req, res) => {
    ...
    for (const [index, id] of ids.entries()) {
      try {
        const result = await schedulePlan(id, dbPool, { force: false });
      ...
      if (index < ids.length - 1) {
        ...
        requestLogger(req, 'pemasaran').info({ delayMs }, 'Bulk Repliz schedule delay');
  ...
  app.post('/api/pemasaran/:id/repliz/schedule', requireLogin, async (req, res) => {
    try {
      const result = await schedulePlan(req.params.id, dbPool, { force: false });
      res.json({
        message: 'Rencana berhasil dijadwalkan ke Repliz',
        plan: result.plan,
        repliz: result.repliz,
      });
    } catch (err) {
      const status = err.statusCode || 500;
      requestLogger(req, 'pemasaran').error({ err }, 'POST /api/pemasaran/:id/repliz/schedule error');
  ...
  app.post('/api/pemasaran/:id/repliz/retry', requireLogin, async (req, res) => {
    try {
      const result = await schedulePlan(req.params.id, dbPool, { force: true });
      res.json({ message: 'Retry Repliz berhasil', plan: result.plan, repliz: result.repliz });
    } catch (err) {
      const status = err.statusCode || 500;
      requestLogger(req, 'pemasaran').error({ err }, 'POST /api/pemasaran/:id/repliz/retry error');
  ...
  app.post('/api/pemasaran/:id/repliz/sync', requireLogin, async (req, res) => {
    try {
      const result = await syncPlan(req.params.id, dbPool);
      res.json({ message: 'Status Repliz berhasil disinkronkan', plan: result.plan, repliz: result.repliz });
    } catch (err) {
      const status = err.statusCode || 500;
      requestLogger(req, 'pemasaran').error({ err }, 'POST /api/pemasaran/:id/repliz/sync error');
  ...
  app.delete('/api/pemasaran/:id', requireLogin, async (req, res) => {
    try {
      const result = await dbPool.query('DELETE FROM pemasaran WHERE id = $1 RETURNING id', [req.params.id]);
      ...
      requestLogger(req, 'pemasaran').error({ err }, 'DELETE /api/pemasaran error');
...
export function registerReplizRoutes(app) {
  app.get('/api/repliz/accounts', requireLogin, async (req, res) => {
    try {
      const type = String(req.query.type || req.query.channel || 'threads')
        .trim()
        .toLowerCase();
      const channel = getChannel(type);
      const data = await channel.listAccounts({
      ...
      requestLogger(req, 'pemasaran').error({ err }, 'GET /api/repliz/accounts error');
