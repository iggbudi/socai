...
import { getReplizSchedule } from '../../shared/repliz.js';
import { getChannel, normalizeChannelId, getDefaultChannelId } from '../channels/index.js';
import { wibDate } from '../../shared/wibTime.js';
...
export function parseMarketingSchedule(plan) {
  ...
  if (isoLike) {
    ...
    // Diinterpretasikan sebagai WIB eksplisit (A4) — bukan server-local time.
    const date = wibDate(Number(y), Number(m), Number(d), Number(hh), Number(mm));
  ...
  if (idDate) {
    ...
    if (Number.isFinite(day) && month !== undefined && Number.isFinite(year)) {
      // bulanIndonesia 0-based → wibDate bulan 1-based (A4).
      const date = wibDate(year, month + 1, day, hour, minute);
      if (!Number.isNaN(date.getTime())) return date;
    }
  }
...
export function normalizePlan(rawPlan, index) {
  const rawGambar = toPlanText(
    rawPlan?.gambar || rawPlan?.image || rawPlan?.image_url || rawPlan?.url_gambar,
  );
  let gambar = '';
  if (rawGambar) {
  ...
  return {
    ...
    kanal: normalizeChannelId(rawPlan?.kanal, { defaultId: getDefaultChannelId() }),
        ...
        const duplicate = await client.query(
          `SELECT id, judul, jadwal FROM pemasaran
           WHERE lower(regexp_replace(coalesce(jadwal, ''), '\\s+', ' ', 'g')) = $1
             AND lower(coalesce(kanal, '')) = $2
           LIMIT 1`,
          [scheduleKey, plan.kanal],
        );
        if (duplicate.rows.length > 0) {
          throw new Error(
            `Jadwal sudah ada di pemasaran ${plan.kanal}: ${plan.jadwal}. Buat jadwal lanjutan agar tidak menumpuk.`,
          );
        }
      }
...
export async function syncPlanReplizStatus(planId, dbPool) {
  ...
  const localStatus =
    replizStatus === 'success' ? 'posted' : replizStatus === 'error' ? 'failed' : 'scheduled';
...
export async function schedulePlanToRepliz(
  planId,
  dbPool,
  { force = false, scheduleAtOverride = null } = {},
) {
  return schedulePlanToChannel(planId, dbPool, { force, scheduleAtOverride });
}

export async function schedulePlanToChannel(
  planId,
  dbPool,
  { force = false, scheduleAtOverride = null } = {},
) {
  ...
  try {
    ...
    plan = locked.rows[0];
    channel = getChannel(plan.kanal || getDefaultChannelId());
    if (!channel.isConfigured()) {
      const err = new Error(channel.configurationError());
      err.statusCode = 400;
      throw err;
    }
    ...
    if (scheduleAtOverride) {
      ...
      if (!scheduledAt) {
        const err = new Error(
          'Jadwal tidak bisa diparse. Gunakan format seperti "2026-06-05 19:00" atau "5 Juni 2026 jam 19:00".',
        );
        err.statusCode = 400;
        throw err;
      }
    ...
    const replizResponse = await channel.createSchedule(
