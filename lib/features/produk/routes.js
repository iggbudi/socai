...
export function registerProdukRoutes(app, { dbPool = pool } = {}) {
  app.get('/api/produk', requireLogin, async (req, res) => {
    try {
      const result = await dbPool.query('SELECT * FROM produk ORDER BY id DESC');
      res.json(result.rows);
    } catch (err) {
      requestLogger(req, 'produk').error({ err }, 'GET /api/produk error');
  ...
  app.get('/api/produk/:id', requireLogin, async (req, res) => {
    try {
      const result = await dbPool.query('SELECT * FROM produk WHERE id = $1', [req.params.id]);
    ...
    try {
      const result = await dbPool.query(
        ...
        [
          nama,
          parsedHarga,
          Number.isFinite(parsedStok) && parsedStok >= 0 ? parsedStok : 0,
          sanitizedGambar,
          deskripsi || '',
        ],
      );
      ...
      requestLogger(req, 'produk').error({ err }, 'POST /api/produk error');
  ...
  app.put('/api/produk/:id', requireLogin, async (req, res) => {
    ...
    try {
      ...
      requestLogger(req, 'produk').error({ err }, 'PUT /api/produk error');
  ...
  app.delete('/api/produk/:id', requireLogin, async (req, res) => {
    try {
      const result = await dbPool.query('DELETE FROM produk WHERE id = $1 RETURNING id', [req.params.id]);
      if (result.rows.length === 0) return res.status(404).json({ error: 'Produk tidak ditemukan' });
      res.json({ message: 'Produk berhasil dihapus' });
    } catch (err) {
      requestLogger(req, 'produk').error({ err }, 'DELETE /api/produk error');
