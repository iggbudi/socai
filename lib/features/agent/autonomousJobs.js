...
import { runAgentTask, CRON_WEEKLY_SESSION_KEY } from './runner.js';
import { purgeOldAgentRuns } from './runs.js';
import { refreshPublishFeedback } from './publishFeedback.js';
import { autoPlanCronIntervalMs, autoPlanMinGaps, agentRunsPurgeIntervalMs } from './autonomousConfig.js';
...
export async function shouldGenerateWeeklyPlans(readPool) {
  const { aiReadPool } = await import('../../shared/db.js');
...
export async function generateWeeklyPlans() {
  ...
  log.info({ gapCount: check.gapCount, autonomyMode: cronMode }, 'Starting weekly plan cron');
...
export async function runPublishFeedbackRefresh(readPool) {
  try {
    const { aiReadPool } = await import('../../shared/db.js');
    const pool = readPool || aiReadPool;
    const summary = await refreshPublishFeedback(pool);
    return { ok: true, summary };
  } catch (err) {
    log.error({ err }, 'Publish feedback refresh error');
    return { ok: false, error: err.message };
  }
}

export async function runAgentRunsPurge(dbPool) {
  const retainDays = Number(process.env.AGENT_RUNS_RETAIN_DAYS) || 90;
  try {
    const { pool } = await import('../../shared/db.js');
    const p = dbPool || pool;
    const deleted = await purgeOldAgentRuns(p, { retainDays });
    if (deleted > 0) {
      log.info({ deleted, retainDays }, 'Purged old agent runs');
    }
    return { deleted, retainDays };
  } catch (err) {
    log.error({ err }, 'Agent runs purge error');
