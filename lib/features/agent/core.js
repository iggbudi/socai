...
function buildActuatorPromptSection(autonomyMode) {
...
async function buildActuatorContext(sessionKey) {
  ...
  const schedulesToday = await countAgentSchedulesToday(pool).catch(() => 0);
  return {
    autonomyMode,
    source,
    sessionKey,
...
async function runActuatorTool(sessionKey, toolName, fn) {
  ...
  try {
    ...
    if (toolName === 'save_content_plan' && result?.ids?.length) {
      const actx = await buildActuatorContext(sessionKey);
      await handlePostSaveApproval(pool, result, { autonomyMode: actx.autonomyMode }).catch((err) => {
        log.error({ err, sessionKey }, 'Schedule approval post-save hook error');
      });
    }
    if (runCtx?.id) {
      await logToolCall(pool, runCtx.id, {
        name: toolName,
...
async function createAgentSessionForKey(sessionKey) {
  ...
  const modelCandidates = [
    ...(isTelegramSession
      ? [process.env.TELEGRAM_AI_MODEL, ...splitModelRefs(process.env.TELEGRAM_AI_MODEL_FALLBACKS)]
      : []),
  ...
  const dbQueryTool = defineTool({
    ...
    execute: async (_toolCallId, params) => {
      ...
      try {
        const result = await aiReadPool.query(query);
  ...
  const getCalendarGapsTool = defineTool({
    ...
    execute: async (_toolCallId, params) =>
      runActuatorTool(sessionKey, 'get_calendar_gaps', async () =>
        getCalendarGaps(aiReadPool, {
  ...
  const saveContentPlanTool = defineTool({
    ...
    execute: async (_toolCallId, params) =>
      runActuatorTool(sessionKey, 'save_content_plan', async () => {
        const context = await buildActuatorContext(sessionKey);
        return saveContentPlan(pool, context, { plans: params.plans });
      }),
  });
  ...
  const scheduleContentTool = defineTool({
    ...
    execute: async (_toolCallId, params) =>
      runActuatorTool(sessionKey, 'schedule_content', async () => {
        const context = await buildActuatorContext(sessionKey);
        return scheduleContent(pool, context, {
  ...
  const syncContentStatusTool = defineTool({
    ...
    execute: async (_toolCallId, params) =>
      runActuatorTool(sessionKey, 'sync_content_status', async () => {
        const context = await buildActuatorContext(sessionKey);
        return syncContentStatus(pool, context, { pemasaran_id: params.pemasaran_id });
      }),
  });
