import { schedulePlanToRepliz } from '../pemasaran/domain.js';
import { getAutonomyConfig } from './actuator/policy.js';
import { notifyTelegramOperators } from '../../shared/telegramNotify.js';
...
export async function notifyScheduleApprovalRequest(plans, { notify = notifyTelegramOperators } = {}) {
...
export async function handlePostSaveApproval(dbPool, { ids = [] } = {}, { autonomyMode, notify } = {}) {
  ...
  const notification = await notifyScheduleApprovalRequest(marked, { notify });
  return { requested: true, marked, notify: notification };
}

export async function approvePlanSchedule(dbPool, planId, { schedule = schedulePlanToRepliz } = {}) {
  ...
  return schedule(id, dbPool, { force: false });
}
