...
export async function refreshPublishFeedback(readPool) {
  const channelIds = getEnabledChannelIds();
  const result = await readPool.query(
    `SELECT id, judul, kanal, status, repliz_status, published_at, repliz_last_error, repliz_synced_at
     FROM pemasaran
  ...
  const posted = rows.filter(
    (r) => (r.repliz_status || '').toLowerCase() === 'success' || r.status === 'posted',
  ).length;
  const failed = rows.filter(
    (r) => (r.repliz_status || '').toLowerCase() === 'error' || r.status === 'failed',
  ).length;
  ...
  const highlights = rows.slice(0, 8).map((r) => {
    ...
    return `#${r.id} [${r.kanal || '?'}:${st}] ${(r.judul || '').slice(0, 60)}${err}`;
  });
