import { schedulePlanToRepliz, syncPlanReplizStatus } from '../../pemasaran/domain.js';
import { checkSchedulePolicy, checkSyncPolicy } from './policy.js';

export async function scheduleContent(
  dbPool,
  context,
  { pemasaran_id, force = false } = {},
  ...
  try {
    const result = await schedulePlan(planId, dbPool, { force: Boolean(force) });
...
export async function syncContentStatus(
  ...
  try {
    const result = await syncPlan(planId, dbPool);
    const plan = result.plan || result;
    return {
      pemasaran_id: plan.id ?? planId,
