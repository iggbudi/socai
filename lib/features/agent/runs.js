...
export async function logToolCall(
  ...
  const ids = Array.isArray(pemasaran_ids)
    ? pemasaran_ids.filter((id) => Number.isFinite(Number(id))).map(Number)
    : [];
...
export async function completeAgentRun(dbPool, runId, { status = 'completed', error_message = null } = {}) {
  if (!runId) return null;
...
export async function getAgentRunMetrics(dbPool, options = {}) {
  const { getEvaluationMetrics } = await import('../evaluasi/metrics.js');
  return getEvaluationMetrics(dbPool, options);
}
