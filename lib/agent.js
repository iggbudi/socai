...
import { buildChannelsPromptSection } from './features/channels/index.js';
import {
  getActiveAgentRunContext,
  logToolCall,
  countAgentSchedulesToday,
} from './agentRuns.js';
import { buildPublishFeedbackPromptSection } from './publishFeedback.js';
import { handlePostSaveApproval } from './scheduleApproval.js';
...
function buildActuatorPromptSection(autonomyMode) {
  ...
  if (mode === 'assistive') {
    ...
    if (String(process.env.REQUIRE_APPROVAL || 'false').toLowerCase() === 'true') {
      lines.push('- REQUIRE_APPROVAL aktif: setelah save, operator mendapat notifikasi Telegram; jangan panggil schedule_content.');
    }
  }
  ...
  try {
    ...
    if (runCtx?.id) {
      await logToolCall(pool, runCtx.id, {
        ...
        pemasaran_ids: toolName === 'save_content_plan'
          ? (result.ids || [])
...
async function createAgentSessionForKey(sessionKey) {
  ...
  const getCalendarGapsTool = defineTool({
    ...
    execute: async (_toolCallId, params) => runActuatorTool(sessionKey, 'get_calendar_gaps', async () => getCalendarGaps(aiReadPool, {
  ...
  const saveContentPlanTool = defineTool({
    ...
    execute: async (_toolCallId, params) => runActuatorTool(sessionKey, 'save_content_plan', async () => {
  ...
  const scheduleContentTool = defineTool({
    ...
    execute: async (_toolCallId, params) => runActuatorTool(sessionKey, 'schedule_content', async () => {
      const context = await buildActuatorContext(sessionKey);
  ...
  const syncContentStatusTool = defineTool({
    ...
    execute: async (_toolCallId, params) => runActuatorTool(sessionKey, 'sync_content_status', async () => {
      const context = await buildActuatorContext(sessionKey);
...
${buildChannelsPromptSection()}${buildActuatorPromptSection(actuatorMode)}${buildPublishFeedbackPromptSection()}`;
  ...
  const allToolNames = [
    dbQueryTool.name,
    webSearchTool.name,
    ...actuatorTools.map((tool) => tool.name),
  ];
