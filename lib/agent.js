...
function buildActuatorPromptSection(autonomyMode) {
  ...
  if (mode === 'assistive') {
    ...
    lines.push(
      ...
      '- Urutan disarankan: \`get_calendar_gaps\` → rencana → \`save_content_plan\` → \`schedule_content\`.',
      '- Jangan klaim sudah simpan/jadwal tanpa tool sukses.',
    );
  }
...
async function createAgentSessionForKey(sessionKey) {
  ...
  const actuatorSource = resolveSourceFromSessionKey(sessionKey);
  const actuatorMode = resolveAutonomyMode(actuatorSource);
...
${buildActuatorPromptSection(actuatorMode)}`;
