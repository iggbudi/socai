...
// Pengecualian terdokumentasi (vertical slicing F2): env.js adalah bootstrap validator,
// bukan bagian lib/shared/ — membaca CHANNEL_IDS dari fitur channels (tanpa duplikasi konstanta).
import { CHANNEL_IDS } from './features/channels/index.js';
import { childLogger, LOG_LEVELS } from './shared/logger.js';
...
const AUTONOMY_MODES = ['assistive', 'supervised', 'bounded'];
...
function validateAutonomyModeEnv(env, name, errors) {
  const value = env[name];
  if (value === undefined || value === '') return;
  const normalized = String(value).trim().toLowerCase();
  if (!AUTONOMY_MODES.includes(normalized)) {
    errors.push(`${name} harus salah satu dari: ${AUTONOMY_MODES.join(', ')}.`);
  }
}

function validateBooleanEnv(env, name, errors) {
  const value = env[name];
  if (value === undefined || value === '') return;
  const normalized = String(value).trim().toLowerCase();
  if (!['true', 'false', '1', '0', 'yes', 'no'].includes(normalized)) {
    errors.push(`${name} harus boolean (true/false).`);
  }
}

function validateLogLevelEnv(env, name, errors) {
  const value = env[name];
  if (value === undefined || value === '') return;
  const normalized = String(value).trim().toLowerCase();
  if (!LOG_LEVELS.includes(normalized)) {
    errors.push(`${name} harus salah satu dari: ${LOG_LEVELS.join(', ')}.`);
  }
}

function validateEnabledChannelsEnv(env, name, errors) {
  const value = env[name];
  if (value === undefined || value === '') return;
  ...
  if (requested.length === 0) {
    errors.push(`${name} tidak boleh kosong jika diisi.`);
    return;
  }
  const unknown = requested.filter((id) => !CHANNEL_IDS.includes(id));
  if (unknown.length > 0) {
    errors.push(
      `${name} berisi kanal tidak dikenal: ${unknown.join(', ')}. Kanal tersedia: ${CHANNEL_IDS.join(', ')}.`,
    );
  }
}

export function collectWebEnvironmentIssues(env = process.env) {
  const errors = [];
  const warnings = [];
  ...
  if (isProduction && !env.SESSION_SECRET) {
    ...
    warnings.push(
      'SESSION_SECRET tidak diisi - memakai secret random sementara (session hilang saat restart).',
    );
  }
  ...
  if (
    usesProvider(
      'xiaomi',
      env.AI_MODEL,
      env.AI_MODEL_FALLBACKS,
      env.TELEGRAM_AI_MODEL,
      env.TELEGRAM_AI_MODEL_FALLBACKS,
    ) &&
    !env.XIAOMI_API_KEY
  ) {
  ...
  validatePositiveIntegerEnv(env, 'WEB_AI_RATE_LIMIT', errors);
  validatePositiveIntegerEnv(env, 'WEB_AI_RATE_WINDOW_MS', errors);

  validateAutonomyModeEnv(env, 'AUTONOMY_MODE', errors);
  validateAutonomyModeEnv(env, 'WEB_AUTONOMY_MODE', errors);
  validateAutonomyModeEnv(env, 'TELEGRAM_AUTONOMY_MODE', errors);
  validateAutonomyModeEnv(env, 'AUTO_PLAN_CRON_AUTONOMY_MODE', errors);
  validateBooleanEnv(env, 'REQUIRE_APPROVAL', errors);
  validateLogLevelEnv(env, 'LOG_LEVEL', errors);
  validatePositiveIntegerEnv(env, 'MAX_AGENT_SAVES_PER_RUN', errors);
  validatePositiveIntegerEnv(env, 'MAX_AGENT_SCHEDULES_PER_DAY', errors);
  validatePositiveIntegerEnv(env, 'AGENT_RUNS_RETAIN_DAYS', errors);
  validatePositiveIntegerEnv(env, 'AUTO_PLAN_CRON_INTERVAL_MS', errors);
  validatePositiveIntegerEnv(env, 'AUTO_PLAN_MIN_GAPS', errors);
  validatePositiveIntegerEnv(env, 'AGENT_RUNS_PURGE_INTERVAL_MS', errors);
  validateEnabledChannelsEnv(env, 'ENABLED_CHANNELS', errors);

  if (isProduction && !env.DB_AI_READ_USER) {
    warnings.push(
      'DB_AI_READ_USER tidak diisi - db_query memakai DB_USER penuh (tidak disarankan production)',
    );
  }
...
function reportEnvironmentIssues({ errors, warnings }, { exitOnError = true } = {}) {
  ...
  if (warnings.length > 0) {
    log.warn({ warnings }, 'Environment warnings');
  }

  if (errors.length > 0) {
    log.error({ errors }, 'Environment errors');
