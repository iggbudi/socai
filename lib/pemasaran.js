...
import { getChannel, normalizeChannelId, getDefaultChannelId } from './features/channels/index.js';
    ...
    for (const plan of plans) {
      ...
      if (scheduleKey) {
        ...
        if (duplicate.rows.length > 0) {
          throw new Error(`Jadwal sudah ada di pemasaran ${plan.kanal}: ${plan.jadwal}. Buat jadwal lanjutan agar tidak menumpuk.`);
        }
      }
...
export async function schedulePlanToChannel(planId, dbPool, { force = false, scheduleAtOverride = null } = {}) {
  const client = await dbPool.connect();
  let plan;
  let channel;
  try {
