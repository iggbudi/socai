...
export function getReplizAccounts({ page = 1, limit = 20, type = 'threads' } = {}) {
  const params = new URLSearchParams({ page: String(page), limit: String(limit), type: String(type || 'threads') });
  ...
  try {
    ...
    if (sanitized.startsWith('/')) {
      if (process.env.APP_URL) {
        try { return new URL(sanitized, process.env.APP_URL).toString(); } catch { return ''; }
      }
      return '';
    }
    return sanitized;
