...
<head>
  ...
  <style nonce="${nonce}">
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .repliz-account-info { font-size: 13px; color: var(--muted); }
    .table-status {
      text-align: center;
      padding: 40px;
      color: var(--muted);
    ...
    .repliz-scheduled-at { font-size: 11px; color: var(--muted); margin-top: 4px; }
    .modal-preview-img { max-width: 100%; margin-top: 8px; border-radius: 8px; }
...
<main class="main">
  ...
  <div class="content">
    <div class="repliz-toolbar">
      <button class="btn btn-detail" id="btn-check-repliz" type="button">Cek Akun Repliz</button>
      <button class="btn btn-detail" id="btn-bulk-repliz" type="button">Bulk Jadwalkan Repliz</button>
      <span id="repliz-account-info" class="repliz-account-info"></span>
    </div>
    <div class="table-wrap">
      <table>
        <thead>
          <tr>
            <th><input type="checkbox" id="select-all-plans"></th>
            <th>Judul</th>
            <th>Kanal</th>
        ...
        <tbody id="pemasaran-list">
          <tr><td colspan="9" class="table-status">Memuat data...</td></tr>
        </tbody>
      </table>
    </div>
  </div>
...
<div class="modal-overlay" id="detail-modal">
  <div class="modal">
    ...
    <div class="modal-close"><button type="button" id="btn-modal-close">Tutup</button></div>
  </div>
</div>
...
async function loadPlans() {
  try {
    ...
    document.getElementById('pemasaran-list').innerHTML =
      '<tr><td colspan="9" class="table-status table-status--error">Gagal memuat data: ' + esc(e.message || 'Terjadi kesalahan') + '</td></tr>';
  }
}
...
function renderPlans() {
  ...
  if (plans.length === 0) {
    tbody.innerHTML = '<tr><td colspan="9" class="table-status">Belum ada rencana pemasaran.<br>Buat rencana melalui Asisten AI.</td></tr>';
    return;
  }
  tbody.innerHTML = plans.map(p => {
    ...
    return '<tr>' +
      ...
      '<td>' + date + '</td>' +
      '<td>' +
        '<button class="btn btn-detail pemasaran-action" data-action="detail" data-id="' + p.id + '">Detail</button> ' +
        replizActionButton(p) + ' ' +
        '<button class="btn btn-danger pemasaran-action" data-action="delete" data-id="' + p.id + '">Hapus</button>' +
      '</td>' +
    '</tr>';
  }).join('');
}
...
function replizBadge(p) {
  ...
  if (p.repliz_schedule_id) {
    ...
    return '<span class="badge ' + cls + '" title="' + esc(title) + '">Repliz: ' + esc(status) + '</span>' +
      '<div class="repliz-scheduled-at">' + esc(fmtDateTime(p.repliz_scheduled_at)) + '</div>';
  }
...
function replizActionButton(p) {
  if (p.repliz_schedule_id) return '<button class="btn btn-detail pemasaran-action" data-action="syncRepliz" data-id="' + p.id + '">Sync Status</button>';
  if (p.repliz_status === 'error') return '<button class="btn btn-detail pemasaran-action" data-action="retryRepliz" data-id="' + p.id + '">Retry Repliz</button>';
  return '<button class="btn btn-detail pemasaran-action" data-action="scheduleRepliz" data-id="' + p.id + '">Jadwalkan Repliz</button>';
}
...
async function showDetail(id) {
  ...
  if (p.gambar) {
    gambarEl.innerHTML = '<a href="' + esc(p.gambar) + '" target="_blank" rel="noopener">Lihat gambar</a><br><img src="' + esc(p.gambar) + '" alt="Gambar konten" class="modal-preview-img">';
  } else {
...
document.getElementById('btn-check-repliz').addEventListener('click', checkReplizAccounts);
document.getElementById('btn-bulk-repliz').addEventListener('click', bulkScheduleRepliz);
document.getElementById('select-all-plans').addEventListener('change', function() {
  toggleAllPlans(this.checked);
});
document.getElementById('btn-modal-close').addEventListener('click', closeModal);
document.getElementById('pemasaran-list').addEventListener('click', (e) => {
  const btn = e.target.closest('.pemasaran-action');
  if (!btn) return;
  const id = Number(btn.dataset.id);
  const action = btn.dataset.action;
  if (action === 'detail') showDetail(id);
  else if (action === 'delete') deletePlan(id);
  else if (action === 'scheduleRepliz') scheduleRepliz(id);
  else if (action === 'syncRepliz') syncRepliz(id);
  else if (action === 'retryRepliz') retryRepliz(id);
});
${HAMBURGER_BIND_JS}
