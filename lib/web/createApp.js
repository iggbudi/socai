...
import { createLoginRateLimiter } from '../features/auth/loginRateLimit.js';
import { registerAuthRoutes } from '../features/auth/routes.js';
import { registerPageRoutes } from './routes/pages.js';
import { registerHealthRoutes } from './routes/health.js';
import { registerProdukRoutes, registerUploadRoutes } from '../features/produk/routes.js';

import { registerPemasaranRoutes, registerReplizRoutes } from '../features/pemasaran/routes.js';

import { registerChannelsRoutes } from '../features/channels/routes.js';
import { registerAsistenRoutes, registerAgentRunsRoutes } from '../features/agent/routes.js';
import { registerEvaluasiRoutes } from '../features/evaluasi/routes.js';
...
export function createWebApp() {
  ...
  // A6 (audit): hanya percaya proxy loopback (Apache di mesin yang sama),
  // bukan semua proxy — X-Forwarded-For/Proto dari luar tidak dipercaya.
  ...
  app.use((req, res, next) => {
    ...
    req.log = requestLogger(req, 'web.request');
    next();
  });
  ...
  registerEvaluasiRoutes(app);
  registerHealthRoutes(app);

  app.use((err, req, res, next) => {
    requestLogger(req, 'web').error({ err }, 'Unhandled error');
    if (res.headersSent) return next(err);

    const isApi = req.path.startsWith('/api/');
    const message =
      err?.code === 'LIMIT_FILE_SIZE' ? 'File terlalu besar. Maksimal 5MB.' : 'Terjadi kesalahan server.';

    if (isApi) return res.status(500).json({ error: message });
    return res
      .status(500)
      .type('html')
      .send(loginPage(message, { nonce: res.locals.cspNonce }));
  });
