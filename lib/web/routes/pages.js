...
export function registerPageRoutes(app) {
  app.get('/dashboard', requireLogin, (req, res) => {
    ...
    res
      .type('html')
      .send(dashboardPage(req.session.user.username, csrfToken, { nonce: res.locals.cspNonce }));
  });
  ...
  app.get('/pemasaran', requireLogin, (req, res) => {
    ...
    res
      .type('html')
      .send(pemasaranPage(req.session.user.username, csrfToken, { nonce: res.locals.cspNonce }));
  });
