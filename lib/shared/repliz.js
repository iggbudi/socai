...
export async function replizFetch(path, options = {}) {
  ...
  const { timeoutMs = DEFAULT_TIMEOUT_MS, headers = {}, body, ...fetchOptions } = options;
...
export function getReplizAccounts({ page = 1, limit = 20, type = 'threads' } = {}) {
  const params = new URLSearchParams({
    page: String(page),
    limit: String(limit),
    type: String(type || 'threads'),
  });
  return replizFetch(`/public/account?${params.toString()}`, { method: 'GET' });
}

export function getThreadsAccounts(options = {}) {
  return getReplizAccounts({ ...options, type: 'threads' });
}
...
function normalizeScheduleAt(plan, options) {
  ...
  if (!date || Number.isNaN(date.getTime())) {
    throw new Error(
      'scheduleAt Repliz tidak valid/ belum tersedia. Isi options.scheduleAt atau kolom scheduled_at/repliz_scheduled_at dengan tanggal ISO yang valid.',
    );
  }
...
function resolveImageUrl(plan, options) {
  ...
  try {
    const sanitized = sanitizeImageUrl(raw, { allowEmpty: true });
    if (!sanitized) return '';
    if (sanitized.startsWith('/')) {
      if (process.env.APP_URL) {
        try {
          return new URL(sanitized, process.env.APP_URL).toString();
        } catch {
          return '';
        }
      }
...
function splitMarketingText(value, maxLength = 500) {
  ...
  while (remaining.length > maxLength) {
    ...
    const cutAt =
      smartCut > Math.floor(maxLength * 0.6) ? smartCut + (remaining[smartCut] === ' ' ? 0 : 1) : maxLength;
...
export function buildThreadsSchedulePayload(plan, options = {}) {
  ...
  const contentParts = splitMarketingText(
    plan?.copywriting || plan?.strategi || plan?.description || '',
    500,
  );
  ...
  const replies =
    options.replies ||
    contentParts.slice(1).map((description) => ({
      description,
      type: 'text',
      medias: [],
    }));
...
export function getReplizSchedule(scheduleId, options = {}) {
  ...
  return replizFetch(`/public/schedule/${encodeURIComponent(scheduleId)}`, {
    method: 'GET',
