...
test('resolveNotifyMinRole tidak pernah naik di atas operator (perilaku saat ini)', () => {
  // Akumulator reduce diinisialisasi 'operator', sehingga peran yang lebih tinggi
  // tidak pernah menang. Konsekuensinya: menyetel TELEGRAM_APPROVAL_NOTIFY_ROLES
  // ke 'super_admin' saja TETAP menotifikasi operator. Dikunci di sini apa adanya;
  // perubahannya masuk backlog, bukan diselundupkan lewat sprint seam DI.
  assert.equal(resolveNotifyMinRole('super_admin'), 'operator');
});
