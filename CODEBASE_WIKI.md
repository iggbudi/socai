...
| S34-D1 | `b120d60` | `lib/features/agent/core.js` (565 baris, permukaan tak-teruji terbesar) diberi seam DI `resolveCoreDeps()` untuk SDK `pi-coding-agent` + pool DB (default = perilaku lama); `test/helpers/fakeAgentSdk.mjs` (baru) + 35 test perilaku (`core.test.js`) → coverage `core.js` 9,20% → 98,20% line; agregat proyek 90,76/81,80/84,38 |
